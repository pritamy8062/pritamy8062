<!-- GitHub Profile README for Pritam Yadav -->

<h1 align="center">👋 Hi, I'm Pritam Yadav</h1>
<h3 align="center">🚀 Final-year Student | IIIT Gwalior | Full Stack Developer | Problem Solver</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&pause=1000&color=00BFFF&center=true&vCenter=true&width=600&lines=Full+Stack+Developer+%7C+MERN+Stack;LeetCode+Knight+%7C+1200%2B+Problems+Solved;Lifelong+Learner+and+Tech+Explorer" />
</p>


---

### 🧑‍💻 About Me  

🎓 I’m **Pritam Yadav**, a final-year student at **IIIT Gwalior**, pursuing an **Integrated B.Tech + M.Tech in Information Technology**.  
💡 Passionate about **problem-solving, algorithms, and scalable web development**, I love turning ideas into impactful digital experiences.  

💪 I’ve solved **1200+ LeetCode problems (Knight Badge)** and actively participate in **Codeforces** and **CodeChef** contests to sharpen my analytical and algorithmic thinking.  

🌐 Previously, I worked as a **Full Stack Developer Intern at Andwemet**, where I contributed to building and optimizing large-scale web applications using the **MERN stack**.  

📩 Reach me at **pritamy611@gmail.com**  
🔗 [LeetCode Profile](https://leetcode.com/u/pritamy611/)

---

### 💼 Experience

**🚀 Full Stack Developer Intern — Andwemet**  
*October 2023 – January 2024*  

- Designed a **multi-level user onboarding system** with an **admin dashboard** serving **10,000+ real users**.  
- Built **100+ reusable components** with **React.js**, reducing **page load time by 25%**.  
- Fixed critical dashboard bugs using **Unit and Integration Testing**, boosting performance by **30%**.  
- **Tech Stack:** MongoDB, Express.js, React, Node.js (MERN)

---

### 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=cpp,python,javascript,nodejs,express,react,redux,nextjs,mongodb,mysql,git,github,vscode,postman,vercel,netlify&perline=8" />
</p>

---

### 🏆 Achievements

- 🥇 **Knight Badge** on LeetCode  
- 🌍 Ranked **727 worldwide** in *LeetCode Weekly Contest 431*  
- 💯 Solved **1000+ Data Structure & Algorithm problems** on LeetCode  

---

### 📊 GitHub Stats  

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=pritamy8062&show_icons=true&theme=tokyonight" height="160px"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=pritamy8062&theme=tokyonight" height="160px"/>
</p>


---

### 🧩 Let's Connect  

<p align="center">
  <a href="mailto:pritamy611@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"></a>
  <a href="https://leetcode.com/u/pritamy611/"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"></a>
  <a href="https://github.com/pritamy8062"><img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white"></a>
  <a href="https://www.linkedin.com/in/pritam-yadav-284450260/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"></a>
</p>


---

### 🎯 Hobbies & Interests

<p align="center">
  🏋️‍♂️ Gym & Calisthenics <br>
  🏊‍♂️ Swimming <br>
  🏸 Badminton <br>
  🎸 Playing Guitar
</p>


---
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=pritamy8062&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />
</p>


